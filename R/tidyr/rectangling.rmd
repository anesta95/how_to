...
```{r}chars2 %>% select(where(is.list))```

What you do next will depend on the purpose of the analysis. Maybe you want a row for every book and TV series that the character appears in:
...
(Note that the empty titles (`""`) are due to an infelicity in the input `got_chars`: ideally people without titles would have a title vector of length 0, not a title vector length of 1 containing an empty string.)
...
We'll finish off with the most complex list, from [Sharla Gelfand's](https://sharla.party/post/discog-purrr/) discography. We'll start the usual way: putting the list into a single column data frame, and then widening so each component is a column. I also parse the `date_added` column into a real data-time.
...
At this level, we see information about when each disc was added to Sharla's discography, not any information about the disc itself. To do that we need to wide the `basic_information` column:

```{r}discs %>% unnest_wider(basic_information)```
...
```{r}discs %>% unnest_wider(basic_information, names_repair = "unique")```

# [Introduction to dplyr](https://dplyr.tidyverse.org/articles/programming.html)
...
dplyr also supports databases via the dbplyr package, once you’ve installed, read `vignette("dbplyr")` to learn more.
...
Note that `starwars` is a tibble, a modern reimagining of the data frame. It's particularly useful for large datasets because it prints only the first few rows. You can learn more about tibbles at https://tibble.tidyverse.org/; in particular you can convert data frames to tibbles with `as_tibble()`
...
`filter()` allows you to select a subset of rows in a data frame. Like all single verbs, the first arguement is the tibble (or data frame). The second and subsequent arguements refer to variables within that dataframe, selecting rows where the expression is `TRUE`.
...
```{r} starwars %>% arrange(desc(height))```
...
```{r} starwars %>% slice(5:10)```

It is accompanied by a number of helpers for common use cases:

* `slice_head()` and `slice_tail()` select the first or last rows.

```{r} starwars %>% slice_head(n = 3)```

* `slice_sample()` randomly selects rows. Use the option prop to choose a certain proportion of the cases.

```{r} starwars %>% slice_sample(n = 5)```
```{r} starwars %>% slice_sample(prop = 0.1)```

Use `replace = TRUE` to perform a bootstrap sample. If needed, you can weight the sample with the `weight` arguement
...
Often you work with large datasets wtih many columns but only a few are actually of interest to you. `select()` allows you to rapidly zoom in on a useful subset using operations that usually only work on numeric variable positions:
...
You can rename variables with `select()` by using named arguements:

```{r} starwars %>% select(home_world = homeworld)```

But because `select()` drops all the variables not explicitly mentioned, it’s not that useful. Instead, use `rename()`:

```{r} starwars %>% rename(home_world = homeworld)```

### Add new columns with `mutate()`

Besides selecting sets of existing columns, it's often useful to add new columns that are functions of existing columns. This is the job of `mutate()`:

```{r} starwars %>% mutate(height_m = height / 100)```
...
```{r} starwars %>% relocate(sex:homeworld, .before = height)```

### Summarize values with `summarize()`

The last verb is `summarize()`. It collapses a data frame to a single row.

```{r} starwars %>% summarize(height = mean(height, na.rm = T))```
...
Together these properties make it easy to chain together multiple steps to achieve a complex result.
...
Or if you don't want to name the intermediate results, you need to wrap the function calls inside of each other:
...
The dplyr verbs can be classified by the type of operations they accomplish (we sometimes speak of their **semantics**, i.e., their meanings). It's helpful to have a good grasp of the difference between select and mutate operations.

### Selecting operations

One of the appealing features of dplyr is that you can refer to columns from the tibble as if they were regular variables. However, the syntatic uniformity of referring to bare column names hides the semantical differences across all verbs. A column symbol supplied to `select()` does not have the same meaning as the same symbol supplied to `mutate()`.
...
One useful subtlety is that this only applies to bare names and to selecting calls `c(height, mass)` or `height:mass`. In all other cases, the columns of the data frame are not put in scope. This allows you to refer to contextual variables in selection helpers:
...
In the first arguement, `name` represents its own position `1`. In the second arguement, `name` is evaluated in the surrounding context and represents the fifth column.
...
Mutating semantics are quite different from selection semantics. Whereas `select()` expects column names or positions, `mutate()` expects _column vectors_. We will set up a smaller tibble to use for our examples.

```{r} df <- starwars %>% select(name, height, mass)```
...
```{r} mutate(df, "height", 2)```

`mutate()` gets length-1 vectors that it interprets as new columns in the data frame. These vectors are recycled so they match the number of rows. That's why it doesn't make sense to supply expressions like `"height" + 10` to `mutate()`. This amounts to adding 10 to a string! The corect expression is:

```{r} mutate(df, height + 10)```

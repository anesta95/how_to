...
```{r} library(dplyr, warn.conflicts = F)```
...
You can optionally supply "identifier" variables in your call to `rowwise()`. These variables are preserved when you call `summarize()`, so they behave somewhat similarly to the grouping variables passed to `group_by()`:
...
`dplyr::summarize()` makes it really easy to summarize values across rows within one column. When combined with `rowwise()` it also makes it easy to summarize values across one row. To see how, we'll start by making a little dataset:
...
Let's say we want to compute the sum of `w`, `x`, `y`, and `z` for each row. We start by making a row-wise data frame:

```{r}rf <- df %>% rowwise(id)```

We can then use `mutate()` to add a new column to each row, or `summarize()` to return just that one summary:

```{r} 
rf %>% mutate(total = sum(c(w, x, y, z)))
rf %>% summarize(total = sum(c(w, x, y, z)))
```
...
The key difference is that when `mutate()` slices up the columns to pass to `length(y)` the grouped mutate uses `[` and the row-wise mutate uses `[[`. The following code gives a flavor of the differences if you used a for loop:
...
```{r}
mods %>% summarize(rmse = sqrt(mean((pred - data$mpg) ^ 2)))
mods %>% summarize(rsq = summary(mod)$r.squared)
...
`rowwise()` doesn't just work with functions that return a length-1 vector (aka summary functions); it can work with any function if that result is a list. This means that `rowwise()` and `mutate()` provide an elegant way to call a function many times with varying arguements, storing the outputs alongside the inputs.
...
* Without arguement names: you could call functions that input and output data frames using `.` to refer to the "current" group. For example, the following code gets the first row of each group:
...
This has been superseded by `pick()` plus `reframe()`, a variant of `summarize()` that can create multiple rows and columns per group.
...
* With arguements: it worked like `mutate()` but automatically wrapped every element in a list:
...
I now believe that behavior is both too magical and not very useful, and it can be replaced by `summarize()` and `pick()`.

```{r}
mtcars %>%
    group_by(cyl) %>%
    summarize(nrows = nrow(pick(everything())))
```
